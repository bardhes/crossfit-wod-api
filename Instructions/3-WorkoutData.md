...
I've switched some of the instructions around to make the process slightly less verbose. 😁

# Add the Data file and Data Access Layer
...
In `workout.js` paste the following code:
...
# Add the `workoutService.js` file:

Add the file manually, or run the command:

```
touch src/services/workoutService.js
```

In the new file, paste the following code which implements the `getAllWorkouts` method and stubs for the remaiming methods:
...
Replace the code in this file with the following, which:

- adds a call to the workout service in each of the methods
- sends a json response from the `getAllWorkouts`call
...
Now if you visit `localhost:3000/api/v1/workouts/` you should see the json representation of the workouts.
